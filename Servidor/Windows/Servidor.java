...
public class Servidor {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // recogemos el puerto del argumento nº 1, la ip con el nº 0 y configurar el socket del servidor con ambos
        ...
        while (true) {
            ...
            // aqui la logica que he pensado es que el cliente mande un mensaje de manera automatica para que notificar al servidor 
            //de que se ha conectado un cliente nuevo y que hay que pasarle el puerto el cual se usara para la nueva conexion al hilo que se ejecutará
            //para dicho cliente.
            if (mensaje.equals("conectar")) {
                // incrementamos el puerto actual de manera sincronizada para que no pete
...
class HiloServidor implements Runnable {
    ...
    public void run() {
        while (true) {
            ...
            // segun el comando guardado en el primer lugar de tokens, entraremos a un caso u otro comando
            switch (tokens[0]) {
                    case "login":
                        ...
                        break;
                    case "get":
                        // en este caso mandamos archivo de la carpeta del usuario logueado y mandamos al cliente
                        ...
                        break;
                    case "put":
                        // en este caso el cliente nos manda un archivo y se guarda en el servidor, en la carpeta del usuario logueado
                    ...
                    case "list":
                        ...
                        try {
                             // ejecutamos el comando dir en el directorio del usuario
                            ProcessBuilder processBuilder = new ProcessBuilder("cmd", "/c", "dir", usuarioActual);
                            Process process = processBuilder.start();

                            // pillamos la salida del proceso
                            InputStream inputStream = process.getInputStream();
                            ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
                            byte[] buffer = new byte[500];
                            int bytesRead;

                            // y la mostramos la salida del proceso
                        while ((bytesRead = inputStream.read(buffer)) != -1) {
                           outputStream.write(buffer, 0, bytesRead);
                        }

                        // esperamos a que el proceso muera
                        ...
                        responseData = ("error ejecutando 'dir': " + exitCode).getBytes();
                    }
            // cerrar los flujos
            outputStream.close();
            inputStream.close();
             } catch (IOException | InterruptedException e) {
            responseData = ("error ejecutando 'dir': " + e.getMessage()).getBytes();
            }
             }break;
                    case "delete":
                ...
                // mandamos la respuesta al cliente despues de ejecutar la logica del comando ejecutado y guardado en reponseData
    ...
    // lo usamos para loguear un usuario con el archivo encontrado en la misma ubicacion que el servidor
